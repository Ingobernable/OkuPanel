(function (global, factory) {
    ...
    var TimelineChart = function () {
        function TimelineChart(element, data, opts) {
			...
			okulec_d3_locale_config = d3.locale(okulec_d3_locale_config);
			d3.time.format = okulec_d3_locale_config.timeFormat;
			d3.format = okulec_d3_locale_config.numberFormat;
			
			var tickFormat = okulec_d3_locale_config.timeFormat.multi([
            ...
            svg.append('rect').attr('class', 'chart-bounds').attr('x', groupWidth).attr('y', 0).attr('height', height).attr('width', width - groupWidth).style("fill", options.chartBgColor);
            ...
            var intervalTexts = groupIntervalItems.append('text').text(function (d) {
                return d.label;
            }).attr('fill', options.labelFontColor).attr('class', withCustom('interval-text')).attr('y', groupHeight / 2 + 5).attr('x', function (d) {
                return x(d.from);
            });
